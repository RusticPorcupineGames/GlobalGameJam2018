...
var StartScreen = {
    ...
    create: function () {
        ...
        game.stage.filters = [this.filter[FILTER_FILMGRAIN], this.filter[
            FILTER_VIGNETTE]];

        for (var level = 1; level < (totalLevels + 1); level++) {
            let _level = level;
            var xWidthOffset = level * (game.world.width / (totalLevels +
                1));
            var buttonName = 'imgLevel' + level + 'Button';
            game.add.button(xWidthOffset, 250, buttonName, function () {
                return this.startGame(_level);
            }, this, 1, 0).anchor.setTo(0.5, 0);
        };
        ...
        game.add.text((game.world.width / 2), 600,
            "WASD to move.    Avoid doctors.    Infect Patients.",
            style).anchor.setTo(0.5, 0.5);
    },
